...
Console.Out.WriteLine("Running timed tests...");
...
static void Find(Range range)
{
	var stopwatch = Stopwatch.StartNew();
	var (value, sequenceLength) = FindLongestSequence(range);
	stopwatch.Stop();
...
static void FindParallel(Range range)
{
	var stopwatch = Stopwatch.StartNew();
...
static (BigInteger value, int sequenceLength) FindLongestSequence(Range range)
{
	(BigInteger value, int sequenceLength) result = (BigInteger.Zero, 0);
