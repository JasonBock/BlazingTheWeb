   ...
   public static class CollatzSequenceTests
   {
		...
		public static void GenerateSequence()
		{
			...
			Assert.Multiple(() =>
			{
				...
				Assert.That(sequence.Sequence[2], Is.EqualTo(new BigInteger(4)), "1");
				Assert.That(sequence.Sequence[3], Is.EqualTo(new BigInteger(2)), "2");
				Assert.That(sequence.Sequence[4], Is.EqualTo(new BigInteger(1)), "3");
			});
		}
